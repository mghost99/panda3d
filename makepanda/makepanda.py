...
if not PkgSkip("PIRATES"):
    CopyAllHeaders('panda/src/piratesbase')
    CopyAllHeaders('panda/src/seapatch')
...
#
# DIRECTORY: panda/src/seapatch/
#
if not PkgSkip("PIRATES"):
    OPTS=['DIR:panda/src/seapatch', 'BUILDING:PIRATES']
    TargetAdd('p3seapatch_composite1.obj', opts=OPTS, input='seapatch_composite1.cxx')

    OPTS=['DIR:panda/src/seapatch']
    IGATEFILES=GetDirectoryContents('panda/src/seapatch', ["*.h", "*_composite*.cxx"])
    TargetAdd('libp3seapatch.in', opts=OPTS, input=IGATEFILES)
    TargetAdd('libp3seapatch.in', opts=['IMOD:panda3d.pirates', 'ILIB:libp3seapatch', 'SRCDIR:panda/src/seapatch'])
...
if not PkgSkip("PIRATES"):
    ...
    PyTargetAdd('pirates_module.obj', opts=['IMOD:panda3d.pirates', 'ILIB:pirates', 'IMPORT:panda3d.core'])

    PyTargetAdd('pirates.pyd', input='pirates_module.obj')
    PyTargetAdd('pirates.pyd', input='libp3seapatch_igate.obj')
    PyTargetAdd('pirates.pyd', input='libp3pirates.dll')
    PyTargetAdd('pirates.pyd', input=COMMON_PANDA_LIBS)
