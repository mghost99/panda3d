...
{
    // Use try_lock to avoid blocking - if we can't get the lock, skip this frame
    if (!m_lock.try_lock()) {
        return true;  // Skip processing if lock is held (e.g., by collect_geometry)
    }

    // Use a scope guard to ensure unlock on all exit paths
    ...
    // Early exit if no geometry has been collected yet
    if (m_geoms.empty() && m_flat_geoms.empty()) {
        m_lock.unlock();
        return true;
    }
    ...
    if (np.is_empty() || np.get_error_type() != NodePath::ET_ok) {
        m_lock.unlock();
        return true;  // Don't cull, just skip processing
    }

    // Check if anchor and center are valid
    NodePath anchor = m_root->get_anchor();
    NodePath center = m_root->get_center();
    if (anchor.is_empty() || center.is_empty()) {
        m_lock.unlock();
        return true;  // Skip processing if anchor/center not set
    }

    // Make local copies of geometry vectors while holding the lock
    // This allows us to release the lock before doing expensive processing
    pvector<GeomVertexData*> flat_geoms_copy = m_flat_geoms;
    pvector<GeomVertexData*> geoms_copy = m_geoms;
    
    // Release lock before expensive operations
    m_lock.unlock();

    // Now do the expensive processing without holding the lock
    Thread* current_thread = trav->get_current_thread();
    CPT(TransformState) transform = data.get_node_path().get_net_transform(current_thread);

    CPT(TransformState) anchor_trans = anchor.get_net_transform(current_thread);
    anchor_trans = anchor_trans->invert_compose(transform);
    LMatrix4f anchor_mat = anchor_trans->get_mat();

    CPT(TransformState) center_trans = center.get_net_transform(current_thread);
    center_trans = center_trans->invert_compose(transform);
    LMatrix4f center_mat = center_trans->get_mat();

    CPT(TransformState) anchor_rel_trans = anchor.get_transform(np, current_thread);
    LMatrix4f anchor_rel_mat = anchor_rel_trans->get_mat();

    for (pvector<GeomVertexData*>::iterator it = flat_geoms_copy.begin();
         it != flat_geoms_copy.end(); ++it)
          do_flat(*it, anchor_mat, center_mat, anchor_rel_mat, current_thread);

    for (pvector<GeomVertexData*>::iterator it = geoms_copy.begin();
         it != geoms_copy.end(); ++it)
...
{
    // Lock to protect geometry collection from concurrent cull_callback
